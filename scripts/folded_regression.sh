...
if (( $# == 0))
then
echo "Please include the folding method"
exit 1
fi
...
sudo python3 $SCRIPTS_DIR/parse_regression.py $CUR_DIR > reg_results.md
...
--circuit_file $RRGF_DIR/temp_runs/k6_arm_core/arm_core.pre-vpr.blif \
...
# linux_arty, minilitex_arty, picosoc_basys3_full_50
# linux_arty_100, minilitex_arty_100, picosoc_basys3_full_100
...
do
   ...
   $perf_options --output perf.out \
   $VTR_ROOT/vpr/vpr arch.timing.xml $SYM_DIR --circuit_file $SYM_BLIF/$SYM_DIR.eblif \
   ...
   --fix_clusters $SYM_BLIF/place_constr/$SYM_DIR.place \
   --sdc_file $SYM_BLIF/sdc/$SYM_DIR.sdc \
   > $CUR_DIR/$SYM_DIR.log
   cp perf.out $CUR_DIR/EArch_tseng_perf.out
